...
class WandbMixinTest(unittest.TestCase):
    ...
    @patch('fastapp.logging.wandb.Artifact',
    MockWandbArtifact)
    @patch.object(wandb.sdk.wandb_run.Run, 'log_artifact',
    mock_log_artifact)
    def test_log_artifact(self):
        ...
        with open(self.artifact_path/'artifact.txt', 'w') as file:
            file.write('test')
        self.wandb_app.log_artifact(self.artifact_path/'artifact.txt', 'test-file', 'text', upload=True)
    
    @patch('fastapp.logging.wandb.Artifact',
    MockWandbArtifact)
    @patch.object(wandb.sdk.wandb_run.Run, 'log_artifact',
    mock_log_artifact_reference)
    def test_log_artifact_reference(self):
        ...
        self.wandb_app.log_artifact(self.artifact_path/'artifact.txt', 'test-artifact', 'text', upload=False)

    @patch('fastapp.logging.wandb.Artifact',
    MockWandbModelArtifact)
    @patch.object(wandb.sdk.wandb_run.Run, 'log_artifact',
    mock_log_artifact_model)
    def test_save_model(self):

        self.wandb_app.init_run('wandb_test', 
        upload_model=True, mode='disabled',
        config = self.test_config
        )
